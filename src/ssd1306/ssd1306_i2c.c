...
inline int ssd1306_get_font(uint8_t character)
{
    ...
    {
        return 38; // Índice da vírgula
    }
    else if (character == '!')
    {
        return 39; // Índice do ponto de exclamação
    }
    else if (character == '?')
    {
        return 40; // Índice do ponto de interrogação
    }
    else if (character == ':')
    {
        return 41; // Índice dos dois-pontos
    }
    else if (character == ';')
    {
        return 42; // Índice do ponto e vírgula
    }
    else
    {
        return 0; // Nada (espaço ou caractere não suportado)
    }
}
